...
- (void)clear {
    LK_vDSP(vclr)(LKUnwrap(self), self.length);
}


- (void)fill:(LKFloat)value {
    if (value) {
        LK_vDSP(vfill)(&value, LKUnwrap(self), self.length);
    }
    else {
        [self clear];
    }
...
- (void)generateFrom:(LKFloat)start by:(LKFloat)step {
    LK_vDSP(vramp)(&start, &step, LKUnwrap(self), self.length);
}


- (void)generateFrom:(LKFloat)start to:(LKFloat)end {
    LK_vDSP(vgen)(&start, &end, LKUnwrap(self), self.length);
}
...
- (void)setValues:(LKVector *)vector {
    ...
    LK_vDSP(vsadd)(LKUnwrap(vector), &LKZero, LKUnwrap(self), length); //BENCH: Other?
}
